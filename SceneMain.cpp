...
void SceneMain::update(float deltaTime)
{
    ...
    // ============= 添加Boss触发逻辑 =============
    // 检查分数是否达到100分并且Boss还没触发
    if (score >= 100 && !bossTriggered) {
        bossTriggered = true;  // 标记已触发，防止重复触发

        // 保存当前分数到Game类，这样Boss场景能获取
        game.setFinalScore(score);

        // 显示提示信息
        std::cout << "分数达到100分！即将进入Boss战！" << std::endl;

        // 设置延迟切换（3秒后进入Boss场景）
        changeToBossAfterDelay = true;
        bossSceneDelay = 3.0f;  // 3秒延迟，让玩家看到提示
    }

    // 处理延迟切换到Boss场景
    if (changeToBossAfterDelay) {
        bossSceneDelay -= deltaTime;
        if (bossSceneDelay <= 0) {
            changeToBossAfterDelay = false;

            // 切换到Boss场景
            auto sceneBoss = new SceneBoss();
            game.changeScene(sceneBoss);
            return;  // 立即返回，不再执行下面的代码
        }
    }
    // ============= 结束添加 =============
